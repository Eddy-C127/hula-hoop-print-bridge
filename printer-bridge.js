      ...
      async message(ws, message) {
        try {
          ...
          if (data.type === 'print') {
            ...
            // --- DETECT USB / CUPS RAW PRINTING ON MACOS / LINUX ---
            if (ip === 'usb' || (!ip.includes('.') && ip !== 'localhost')) {
              const printerName = ip === 'usb' ? 'Impresora_Predeterminada' : ip;
              console.log(`[Bridge] Impresión USB (CUPS raw) solicitada para: ${printerName}`);
              const tempFile = `temp_ticket_${Date.now()}.bin`;
              try {
                ...
                exec(`lp -d "${printerName}" -o raw "${tempFile}"`, (err, stdout, stderr) => {
                  ...
                  if (err) {
                    console.error("[Bridge] Error al enviar a CUPS USB:", err.message);
                    ws.send(JSON.stringify({ status: 'error', message: `Error en CUPS: ${err.message}` }));
                  } else {
                    console.log("[Bridge] Ticket enviado a la cola USB exitosamente.");
                    ws.send(JSON.stringify({ status: 'success' }));
                  }
                });
              } catch (writeErr) {
                console.error("[Bridge] Error al guardar archivo temporal:", writeErr.message);
                ws.send(JSON.stringify({ status: 'error', message: writeErr.message }));
              }
              return;
            }
            // -------------------------------------------------------
